module Admin::ProductsHelper
    ...
    def select_song_field(name, f, association, options={})
        new_object = f.object.class.reflect_on_association(association).klass.new
        id = new_object.object_id
        ...
        select_fields = f.fields_for(association, new_object, child_index: id) do |builder|
            render("select_song_form", f: f)
        end

        link_to(name, '#', class: "btn btn-default change-mode", data: {mode: "new", id: id, new: new_fields.gsub("\n",""), exist: select_fields.gsub("\n", "")})
    end
